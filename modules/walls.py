...
class WallGenerator():
    def __init__(self, llm):
        self.json_template = {"id": None, "roomId": None,
    ...
    def generate_walls(self, scene):
        # get wall height
        if "wall_height" not in scene: wall_height = self.get_wall_height(scene)
        else: wall_height = scene["wall_height"]
    ...
    def get_wall_height(self, scene):
        ...
        if "wall_height" not in scene:
            ...
            try: wall_height = float(wall_height)
            except: wall_height = round(random.uniform(2.5, 4.5), 1) # if failed, random height between 2.5 and 4.5
    ...
    def get_connected_rooms(self, wall_polygon, rooms, roomId):
        ...
        for room in rooms:
            if room["id"] == roomId: continue # do not consider the room itself
    ...
    def update_walls(self, original_walls, open_room_pairs):
        ...
        open_walls = {"segments": open_wall_segments_no_overlap, "openWallBoxes": open_wall_rectangles}
    ...
    def get_wall_direction(self, wall_endpoint1, wall_endpoint2, room_vertices):
        ...
        wall_center = [(wall_endpoint1[0] + wall_endpoint2[0])/2, (wall_endpoint1[1] + wall_endpoint2[1])/2]
