...
class ObjectSelector:
    ...
    def check_floor_placement(self, candidates, room_vertices, scene):
        ...
        valid_candidates = []
        for candidate in candidates:
            object_size = self.database[candidate[0]]["assetMetadata"]["boundingBox"]
            object_dim = (object_size["x"]*100 + self.size_buffer, object_size["z"]*100 + self.size_buffer)
            ...
            if solutions != []: valid_candidates.append(candidate)
            else: print(f"Floor Object {candidate[0]} (size: {object_dim}) cannot be placed in room"); continue
    ...
    def check_wall_placement(self, candidates, room_vertices, scene):
        ...
        for candidate in candidates:
            object_size = self.database[candidate[0]]["assetMetadata"]["boundingBox"]
            object_dim = (object_size["x"]*100, object_size["y"]*100, object_size["z"]*100)

            solutions = solver.get_all_solutions(room_poly, grid_points, object_dim, height=0)
            solutions = solver.filter_collision(initial_state, solutions)

            if solutions != []: valid_candidates.append(candidate)
            else: print(f"Wall Object {candidate[0]} (size: {object_dim}) cannot be placed in room"); continue
        
        return valid_candidates


    def get_initial_state_floor(self, room_vertices, scene, add_window=True):
        doors, windows, open_walls = scene["doors"], scene["windows"], scene["open_walls"]
        room_poly = Polygon(room_vertices)
        ...
        for door in doors:
            ...
            for door_box in door_boxes:
                ...
                door_center = door_poly.centroid
                if room_poly.contains(door_center):
                    initial_state[f"door-{i}"] = ((door_center.x, door_center.y), 0, door_vertices, 1)
                    i += 1
        ...
        if add_window:
            for window in windows:
                ...
                for window_box in window_boxes:
                    ...
                    window_center = window_poly.centroid
                    if room_poly.contains(window_center):
                        initial_state[f"window-{i}"] = ((window_center.x, window_center.y), 0, window_vertices, 1)
                        i += 1
        ...
        if open_walls != []:
            for open_wall_box in open_walls["openWallBoxes"]:
                ...
                open_wall_center = open_wall_poly.centroid
                if room_poly.contains(open_wall_center):
                    initial_state[f"open-{i}"] = ((open_wall_center.x, open_wall_center.y), 0, open_wall_vertices, 1)
                    i += 1
