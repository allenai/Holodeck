...
class DoorGenerator():
    ...
    def get_door_polygon(self, segment, door_dimension, connection_type):
        ...
        if door_width >= original_length:
            ...
            polygon = [{"x": door_start, "y": 0, "z": 0},
                    {"x": door_end, "y": door_height, "z": 0}]
            
            door_segment = [list(start + normalized_vector * door_start), list(start + normalized_vector * door_end)]
            ...
            if len(connections) != 0:
                for connection in connections:
                    if connection["roomId"] == room1_id:
                        valid_connections.append({"wall0": wall["id"], 
    ...
    def get_room_pairs_str(self, rooms, walls):
        room_pairs = [(wall["roomId"], wall["connected_rooms"][0]["roomId"]) for wall in walls if len(wall["connected_rooms"]) == 1 and wall["width"] >= 2.0]
        for wall in walls:
        ...
        for pair in room_pairs_no_dup:
            ...
            if pair[0] not in existed_rooms: existed_rooms.append(pair[0])
            if pair[1] not in existed_rooms: existed_rooms.append(pair[1])
    ...
    def get_random_door(self, wall_width):
        ...
        if wall_width < 2.0: return random.choice(single_doors)
