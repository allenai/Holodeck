...
class FloorObjectGenerator():
    def __init__(self, llm, object_retriever):
        ...
        self.constraint_prompt = PromptTemplate(input_variables=["room_type", "room_size", "objects"],
    ...
    def generate_objects_per_room(self, args):
        ...
        if use_constraint:
            # get constraints
            constraint_prompt = self.constraint_prompt.format(room_type=room_type,
                                                                room_size=room_size,
            ...
            solution = solver.get_solution(room_poly, objects_list, constraints, initial_state, use_milp=self.use_milp)
            ...
            baseline_prompt = self.baseline_prompt.format(room_type=room_type,
                                                            room_size=room_size,
                                                            objects=", ".join(object_names))
            room_origin = [min(v[0] for v in room['vertices']), min(v[1] for v in room['vertices'])]
            all_is_placed = False
            while not all_is_placed:
                ...
                for data in all_data:
                    ...
                    placement["position"] = {"x": room_origin[0] + (data['position']["X"]/100),
                                                "y": dimension["y"] / 2,
            ...
            for window in windows:
                ...
                for window_box in window_boxes:
                    ...
                    if room_poly.contains(window_center):
                        door_window_placements[f"window-{i}"] = ((window_center.x, window_center.y), 0, window_vertices, 1)
                        i += 1
        ...
        if open_walls != []:
            for open_wall_box in open_walls["openWallBoxes"]:
                ...
                if room_poly.contains(open_wall_center):
                    door_window_placements[f"open-{i}"] = ((open_wall_center.x, open_wall_center.y), 0, open_wall_vertices, 1)
                    i += 1
    ...
    def solution2placement(self, solutions, object_name2id, room_id):
        ...
        for object_name, solution in solutions.items():
            ...
            placement["id"] = f"{object_name} ({room_id})"
            placement["position"] = {"x": solution[0][0] / 100, "y": dimension["y"] / 2, "z": solution[0][1] / 100}
    ...
    def parse_constraints(self, constraint_text, object_names):
        ...
        for plan in plans:
            ...
            for constraint in constraints:
                ...
                try: constraint_type = constraint_name2type[constraint_name]
                except: print(f"constraint type {constraint_name} not found"); continue
                ...
                if constraint_type == "global":
                    ...
                    try: target = constraint.split(",")[1].strip()
                    except: print(f"wrong format of constraint: {constraint}"); continue

                    if target in object2constraints:
                        if constraint_name == "around":
                            object2constraints[object_name].append({"type": "distance", "constraint": "near", "target": target})
                            object2constraints[object_name].append({"type": "direction", "constraint": "face to", "target": target})
                        elif constraint_name == "in front of":
                            object2constraints[object_name].append({"type": "relative", "constraint": "in front of", "target": target})
                            object2constraints[object_name].append({"type": "alignment", "constraint": "center aligned", "target": target})
                        else:
                            object2constraints[object_name].append({"type": constraint_type, "constraint": constraint_name, "target": target})
                    else:
    ...
    def order_objects_by_size(self, selected_floor_objects):
        ...
        ordered_floor_objects_no_size = [[object_name, asset_id] for object_name, asset_id, size in ordered_floor_objects]
